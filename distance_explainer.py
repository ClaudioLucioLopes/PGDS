...
import logging
from utils import move_axis, to_xarray, get_function
import numpy as np
import numpy.typing
from maskers import generate_interpolated_float_masks_for_image
...
class DistanceExplainer:
    ...
    def explain_image_distance(self, model_or_function, input_data, embedded_reference, masks=None) -> tuple[
        ...
        runner = get_function(model_or_function, preprocess_function=full_preprocess_function)
    ...
    def _prepare_input_data(self, input_data):
        input_data_xarray = to_xarray(input_data, self.axis_labels, DistanceExplainer.required_labels)
        ...
        prepared_input_data = move_axis(input_data_xarray_expanded, 'channels', -1)
    ...
    def _prepare_image_data(self, input_data):
        ...
        Returns:
            transformed input data, preprocessing function to use with get_function()
        """
        # ensure channels axis is last and keep track of where it was so we can move it back
    ...
    def _get_full_preprocess_function(self, channel_axis_index, dtype):
        ...
        def moveaxis_function(data):
            return move_axis(data, 'channels', channel_axis_index).astype(dtype).values
